...
class ProfilGuruScreen(Screen):
    ...
    def load_data(self):
        self.guru_data = GuruModel.get_by_user_id(current_user.get('id_user'))
        if self.guru_data:
            self.ids.lbl_nama.text = self.guru_data.get('nama') or "Guru"
            self.ids.lbl_nip.text = self.guru_data.get('nip') or "-"
            self.ids.lbl_no_hp.text = self.guru_data.get('no_hp') or "-"
            
            foto = self.guru_data.get('foto')
            if foto and os.path.exists(foto):
                self.ids.img_avatar.source = foto
            else:
                self.ids.img_avatar.source = "assets/default_avatar.png"
        else:
            self.ids.lbl_nama.text = "Guru"
            self.ids.lbl_nip.text = "-"
            self.ids.lbl_no_hp.text = "-"
            self.ids.img_avatar.source = "assets/default_avatar.png"

    def go_to_edit(self):
        self.manager.transition.direction = 'left'
        self.manager.current = 'edit_profil_guru'
